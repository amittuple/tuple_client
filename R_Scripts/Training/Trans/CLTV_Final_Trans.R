...
#
############################################
print ('CLTV_Final_trans')

toInstallCandidates <- c("BTYD", "data.table", "RPostgreSQL", "Matrix", "gsl", "zoo", "magrittr", "dplyr")
...
trans.lapse = trans.load %>%
  group_by(cust) %>%
  mutate(ltd = max(date))

cust.sltd = trans.lapse %>%
  filter(date < ltd) %>%
  group_by(cust) %>%
...
cust.ltd$cust = as.integer(cust.ltd$cust)
...
pred.cltv$cust = as.integer(pred.cltv$cust)
...
rm(list = c('cal.cbs1', 'cust.lapsed', 'cust.ltd', 'cust.rev', 'cust.sltd', 'cust.val', 'cust.val2', 'event', 'lookup_numeric', 'm.x.value', 'nowd.cal', 'nowd.cal.spend', 'nowd.hold', 'pred.cltv', 'trans.lapse', 'trans.load', 'trans.load.val', 'x.vector'))
